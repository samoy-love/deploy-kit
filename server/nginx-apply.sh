...
# --- Приведение --dest к каноническому виду ДО проверки ---------------------
#
# Белый список ниже — это `case` по glob-шаблонам, а в `case` шаблон `*` матчит
# И СЛЭШ ТОЖЕ. То есть строка вида
#     /etc/nginx/sites-available/../../cron.d/pwn
# шаблону `/etc/nginx/sites-available/*` удовлетворяет — и `install` в конце
# скрипта запишет по этому пути файл от root. sudoers.d/deploy-kit даёт CI
# NOPASSWD ровно на этот скрипт с произвольными аргументами и прямым текстом
# перекладывает проверку аргументов сюда, так что дыра здесь — это root на
# хосте в обмен на утёкший DEPLOY_SSH_KEY.
#
# Поэтому два барьера, оба «падают закрытыми» (сомнение = отказ):
#
#   1. Любое `..` в пути — отказ сразу, без попыток разобрать смысл. Ровно так
#      же устроена проверка NGINX_CONF в bin/deploy.
#   2. Каталог назначения разворачивается физически (`pwd -P` снимает и `.`,
#      и симлинки), и дальше проверяется УЖЕ канонический путь. Значит, к
#      моменту `case` в имени файла слэша быть не может в принципе, и шаблон
#      `/etc/nginx/snippets/samoylove-*.conf` снова означает то, что написано.
case "$DEST" in
    *..*) die "в --dest недопустимо '..' — путь обязан быть прямым: $DEST" ;;
esac
DEST_DIR="$(cd "$(dirname "$DEST")" 2>/dev/null && pwd -P || true)"
[[ -n "$DEST_DIR" ]] || die "каталога назначения не существует: $(dirname "$DEST")"
