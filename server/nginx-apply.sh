...
# --- 0. Один применяющий за раз -------------------------------------------
#
# Всё, что ниже, работает с ОБЩИМ состоянием: `nginx -t` проверяет
# конфигурацию хоста целиком, `systemctl reload` перезагружает один процесс на
# все сайты. Разные проекты трогают разные файлы, но проверяют и перезагружают
# они одно и то же, и без блокировки два одновременных применения путаются в
# показаниях:
#
#   * шаг 1 объявляет «до правки всё исправно», а к шагу 5 сосед уже положил
#     свой файл — и `nginx -t` падает из-за него. Чужая ошибка выглядит как
#     своя, скрипт откатывает СВОЙ исправный конфиг и всё равно возвращает
#     провал;
#   * два `reload` подряд на один процесс — в лучшем случае лишняя работа, в
#     худшем второй приходит в момент, когда первый ещё разбирает конфиг.
#
# Ровно это и случилось при мерже нескольких PR подряд: параллельные выкатки
# перестали быть теорией. Уникальные имена файлов (см. workflow) снимают
# гонку за ВХОДНОЙ файл, а эта блокировка — гонку за /etc/nginx.
#
# Ждём, а не отказываем: соседняя выкатка занимает секунды, и отказ здесь
# означал бы красный прогон на ровном месте. Таймаут на случай, если
# применяющий завис: лучше внятная ошибка, чем job, висящий до отмены.
LOCK=/run/lock/deploy-kit-nginx.lock
exec 9>"$LOCK" || die "не удалось открыть $LOCK"
if ! flock -w 300 9; then
    die "пять минут ждём другую выкатку nginx и не дождались — проверьте, не завис ли применяющий"
fi
