...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        # Незнакомый флаг — НЕ повод ронять выкатку. Пайплайн обновляется
        # сам (`uses: …@main`), а этот скрипт живёт на хосте и обновляется
        # отдельной выкаткой deploy-kit, то есть закономерно отстаёт на
        # минуты. Смерть на неизвестном аргументе превращала это отставание в
        # упавшую выкатку чужого проекта. Ровно так уже устроен
        # publish-file.sh. Обязательное от этого не теряется: без --version и
        # --archive скрипт по-прежнему умирает проверками ниже.
...
# --- Событие выкатки ------------------------------------------------------
#
# Сборка, доставка и защита от влияния на исход живут в notify_event
# (server/lib.sh) — одной реализацией на все серверные скрипты. Здесь только
# точки, где о выкатке есть что сказать.
...
# --- 5. Прибраться --------------------------------------------------------
prune_releases "$RELEASES" "$KEEP" "$CURRENT" "$PREVIOUS"
