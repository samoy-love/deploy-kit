...
# ОКРУЖЕНИЕ GITHUB ACTIONS ВЫЧИЩАЕТСЯ ПЕРЕД КАЖДЫМ ВЫЗОВОМ.
#
# lib/notify.sh определяет source по среде: есть GITHUB_ACTIONS=true — значит
# ci, нет — local (и это правильно, забыть флаг легко, а событие с машины,
# помеченное как пайплайновое, врёт про единственное, что в нём есть про
# происхождение). Случаи «с машины разработчика» опираются на это умолчание и
# передают только DK_RUN_STARTED_MS с DK_RUN_PID.
#
# На раннере GITHUB_ACTIONS, GITHUB_REPOSITORY, GITHUB_RUN_ID и
# GITHUB_RUN_ATTEMPT стоят в окружении сами. Без вычистки локальные случаи
# получали source=ci, а вместе с ним чужие прообразы id и group — и набор
# разваливался пятью расхождениями там, где кода это вовсе не касается. Хуже
# того, три события «в одну миллисекунду» получали ОДИН id (у ci он не зависит
# от времени) и схлопывались писателем в одно как повтор доставки.
#
# Заметить это было неоткуда: набор лежал в репозитории и не запускался ни в
# одном прогоне, то есть проверялся только там, где переменных нет.
#
# Случаи «из пайплайна» выставляют GITHUB_* сами, своим env, и вычистка им не
# мешает: их env выполняется после этого.
run() {
    RC=0
    env -u GITHUB_ACTIONS -u GITHUB_REPOSITORY -u GITHUB_RUN_ID -u GITHUB_RUN_ATTEMPT         DK_NOTIFY_SLEEPS="0 0" "$@" >"$OUT" 2>&1 || RC=$?
}
